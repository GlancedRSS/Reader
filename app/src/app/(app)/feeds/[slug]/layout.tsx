...
import { useFeed, useUserPreferences } from '@/hooks/api'
...
export default function ArticlesLayout({
	...
	const { data: subscriptionDetails } = useFeed(params?.slug as string)
	const { shouldShowSplitLayout } = useSplitLayout()

	useMetadata(`${subscriptionDetails?.title || 'Articles'} | Glanced Reader`)
	...
	const {
		...
		handleTabChange,
		setAnimationState
	} = useDynamicTabNavigation(`/feeds/${params.slug}`)
	...
	return (
		...
		>
			<Header
				feed={subscriptionDetails || undefined}
				title={subscriptionDetails?.title || 'Articles'}
				type='feed'
				website={subscriptionDetails?.website}
			/>
